...
} from "@/components/join-server-dialog";

const ServerButton = memo(
  ({
    server,
    isActive = false,
    onClick,
          ...
          <Button
            ...
            className={cn(
              ...
              isActive
                ...
                : "rounded-3xl hover:rounded-2xl hover:shadow-md hover:shadow-foreground/10"
            )}
        ...
        <div className="flex justify-center relative">
          ...
          >
            ...
            <img
              ...
              alt="Home"
              className={cn(
                "w-10 h-10 object-cover transition-all duration-200 rounded-lg",
                isActive
...
const AddServerButton = ({
  ...
  const handleCreateServer = async () => {
    ...
    try {
      ...
      if (serverIcon) {
        ...
        try {
          iconId = await (connectionStrategy == ConnectionStrategies.ScannedJWK
            ? uploadFileTurbo(serverIcon, jwk)
            : uploadFileTurbo(serverIcon));
          toast.dismiss();
          toast.success("Icon uploaded successfully", {
            ...
            style: {
              ...
              borderRadius: "12px",
              boxShadow:
                "0 10px 25px -5px rgba(34, 197, 94, 0.15), 0 4px 6px -2px rgba(34, 197, 94, 0.1)",
              backdropFilter: "blur(8px)",
            },
            className: "font-medium",
            duration: 3000,
          ...
          toast.error("Failed to upload icon. Creating server without icon.", {
            ...
            style: {
              ...
              borderRadius: "12px",
              boxShadow:
                "0 10px 25px -5px rgba(239, 68, 68, 0.15), 0 4px 6px -2px rgba(239, 68, 68, 0.1)",
              backdropFilter: "blur(8px)",
            },
            className: "font-medium",
            duration: 4000,
      ...
      if (serverId) {
        ...
        try {
          ...
          if (!joinSuccess) {
            ...
            toast.warning(
              "Server created but failed to join automatically. You may need to join manually.",
              {
                richColors: true,
                style: {
                  backgroundColor: "var(--background)",
                  ...
                  borderRadius: "12px",
                  boxShadow:
                    "0 10px 25px -5px rgba(245, 158, 11, 0.15), 0 4px 6px -2px rgba(245, 158, 11, 0.1)",
                  backdropFilter: "blur(8px)",
                },
                className: "font-medium",
                duration: 5000,
        ...
        const currentServers = Array.isArray(serversJoined[address])
        ...
        try {
          const serverDetails = await subspace.server.getServerDetails({
            serverId,
          });
          if (serverDetails) {
            const server: Server = {
  ...
  return (
    <div className="relative group mb-3">
      ...
      <div
        className={cn(
          "absolute -left-3 top-1/2 -translate-y-1/2 w-1.5 rounded-r-full transition-all duration-200 ease-out",
          isHovered
        ...
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              variant="ghost"
              className={cn(
                "w-12 h-12 p-0 transition-all duration-200 ease-out hover:bg-transparent group relative overflow-hidden",
                "before:absolute before:inset-0 before:bg-gradient-to-br before:from-background/10 before:to-transparent before:opacity-0 hover:before:opacity-100 before:transition-opacity before:duration-300",
          ...
          <PopoverContent
            side="right"
            className="w-72 p-0 shadow-xl border-border/50"
          >
                              ...
                              <div className="p-3 bg-muted/30 rounded-lg border border-border/50">
                                ...
                                <ul className="text-xs text-muted-foreground space-y-1">
                                  <li className="flex items-start gap-2">
                                    ...
                                    <span>
                                      Your server will be created on the Arweave
                                      network
                                    </span>
                                  </li>
                                  <li className="flex items-start gap-2">
                                    <div className="w-1 h-1 rounded-full bg-green-500 mt-1.5 flex-shrink-0" />
                                    <span>
                                      You'll be the server owner with full
                                      permissions
                                    </span>
                                  </li>
                                  <li className="flex items-start gap-2">
                                    <div className="w-1 h-1 rounded-full bg-green-500 mt-1.5 flex-shrink-0" />
                                    <span>
                                      You can invite others and create channels
                                    </span>
                                  </li>
                                </ul>
                              </div>
...
export default function ServerList(
  ...
  const { servers, actions, activeServerId, serversJoined, loadingServers } =
    useServer();
    ...
    >
      ...
      <div className="space-y-1 overflow-visible">
              ...
              servers[serverId] && (
                ...
                >
                  <ServerButton
                    ...
                    onClick={() =>
                      actions.setActiveServerIdAndRestoreChannel(serverId)
                    }
                  />
                </div>
              )
        ...
        {(() => {
          ...
          const expectedServerIds = Array.isArray(serversJoined[address])
            ? serversJoined[address]
            : [];

          // Count how many servers have actually been loaded (have data in servers object)
          const loadedServersCount = expectedServerIds.filter(
            (serverId) => servers[serverId]
          ).length;

          // Show skeletons for servers that are expected but not yet loaded
          const skeletonsToShow = loadingServers
            ? Math.max(0, expectedServerIds.length - loadedServersCount)
            : 0;
